...
export default function ChatWindow() {
  ...
  useEffect(() => {
    ...
    newSocket.on("newMessage", (message: Message) => {
      ...
      setTypingIndicators((prevIndicators) =>
        prevIndicators.filter(
          (indicator) => indicator.memberId !== message.senderId
        )
      );
    });
    ...
    newSocket.on(
      "typingIndicator",
      ({ memberId, isTyping }: TypingIndicator) => {
        setTypingIndicators((prevIndicators) => {
          const existingIndex = prevIndicators.findIndex(
            (i) => i.memberId === memberId
          );
          if (existingIndex !== -1) {
            if (isTyping) {
              // Update existing indicator
              return prevIndicators.map((indicator, index) =>
                index === existingIndex ? { ...indicator, isTyping } : indicator
              );
            } else {
              // Remove indicator if not typing
            ...
            // Add new indicator
            return [...prevIndicators, { memberId, isTyping }];
          }
  ...
  const TypingIndicator = ({ memberId }: { memberId: string }) => (
    <div className="flex items-center space-x-2 mb-4">
      <span className="text-xs text-gray-500">{memberId} is typing</span>
  ...
  const setDelay = async (delay: number) => {
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/program/set-chat-delay`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="p-4 bg-gray-200 flex justify-between items-center">
        <div>
          <button onClick={() => setDelay(0)} className={getButtonClass(0)}>
            Fast
          </button>
          <button
            onClick={() => setDelay(5000)}
            className={getButtonClass(5000)}
          >
            Medium
          </button>
          <button
            onClick={() => setDelay(15000)}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {typingIndicators.map((indicator) => (
          <TypingIndicator
            key={indicator.memberId}
            memberId={indicator.memberId}
          />
        ))}
